...
Процедура ВывестиСправку()
	...
	Сообщить("       --ibconnection  [env RUNNER_IBCONNECTION] строка подключения к базе данных");
...
Процедура ОпределитьПараметрыРаботы(Аргументы = Неопределено) Экспорт
		...
		Если ЗначениеЗаполнено(Аргументы.ЗначенияПараметров["--ibconnection"]) Тогда
			Аргументы.ЗначенияПараметров.Вставить("--ibconnection", ПереопределитьПолныйПутьВСтрокеПодключения(Аргументы.ЗначенияПараметров["--ibconnection"]));
		КонецЕсли;
...
Процедура ОсновнаяРабота()
	...
	Попытка
		...
		Парсер.ДобавитьИменованныйПараметр("--ibconnection", "Строка подключения к БД", Истина);
		...
		Если Команда = ВозможныеКоманды().ДекомпилироватьEpf Тогда
			Декомпилировать(ПолныйПуть(Аргументы.ЗначенияПараметров["inputPath"]), ПолныйПуть(Аргументы.ЗначенияПараметров["outputPath"]), "", "",
							Аргументы.ЗначенияПараметров["--ibconnection"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
		...
		ИначеЕсли Команда = ВозможныеКоманды().СобратьИзИсходниковТекущуюКонфигурацию Тогда
			СобратьИзИсходниковТекущуюКонфигурацию(ПолныйПуть(Аргументы.ЗначенияПараметров["inputPath"]),
							Аргументы.ЗначенияПараметров["--ibconnection"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"], Аргументы.ЗначенияПараметров["--v8version"]);
		...
		ИначеЕсли Команда = ВозможныеКоманды().ОбновитьБазу Тогда
			ЗапуститьОбновлениеКонфигурации(Аргументы.ЗначенияПараметров["--ibconnection"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
		...
		ИначеЕсли Команда = ВозможныеКоманды().ЗапуститьВРежимеПредприятия Тогда
			ЗапуститьВРежимеПредприятия(Аргументы.ЗначенияПараметров["--ibconnection"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
		...
		ИначеЕсли Команда = ВозможныеКоманды().ОбновитьИзХранилища Тогда
			ЗапуститьОбновлениеИзХранилища(Аргументы.ЗначенияПараметров["--ibconnection"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
		...
		ИначеЕсли Команда = ВозможныеКоманды().КонвертироватьФайлы Тогда
			КонвертироватьФайлы(ПолныйПуть(Аргументы.ЗначенияПараметров["inputPath"]), Аргументы.ЗначенияПараметров["--ibconnection"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
								Аргументы.ЗначенияПараметров["--v8version"]);
		ИначеЕсли Команда = ВозможныеКоманды().Конфигуратор Тогда
			Конфигуратор(Аргументы.ЗначенияПараметров["--ibconnection"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
						Аргументы.ЗначенияПараметров["--storage-name"], Аргументы.ЗначенияПараметров["--storage-user"], Аргументы.ЗначенияПараметров["--storage-pwd"], Аргументы.ЗначенияПараметров["--storage-ver"],
